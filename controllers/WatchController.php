...
use d2b\watchers\HI_PDB_Announce_Lst;
use d2b\watchers\HI_PDB_OpenNego_Lst;
...
class WatchController extends \yii\console\Controller
{
  public function actionBid(){
    $pdb=new HI_PDB_Announce_Lst;
    $pdb->on(WatchEvent::EVENT_ROW,[$this,'onBidPdbRow']);

    $pdb2=new HI_PDB_OpenNego_Lst;
    $pdb2->on(WatchEvent::EVENT_ROW,[$this,'onBidPdb2Row']);
    ...
    while(true){
      try {
        $this->stdout("==국내조달 공개협상계획==\n",Console::FG_YELLOW);
        $pdb2->watch();
        $this->stdout("==시설공사 입찰공고==\n",Console::FG_YELLOW);
        $peb->watch();
        $this->stdout("==시설공사 공개협상계획==\n",Console::FG_YELLOW);
        $peb2->watch();
        $this->stdout("==국내조달 경쟁입찰공고==\n",Console::FG_YELLOW);
        $pdb->watch();
      }
      catch(\Exception $e){
   ...
   * 국내조달 경쟁입찰공고 row 
   */
  public function onBidPdbRow($event){
    $row=$event->row;
   ...
   * 국내입찰 공개수의협상 공고 row
   */
  public function onBidPdb2Row($event){
    $row=$event->row;
