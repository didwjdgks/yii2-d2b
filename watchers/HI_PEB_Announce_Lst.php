...
class HI_PEB_Announce_Lst extends \yii\base\Component
{
  const URL='http://www.d2b.go.kr/Internet/jsp/peb/HI_PEB_Main.jsp?md=221&cfn=HI_PEB_Announce_Lst';
    ...
    try{
      ...
      for($page=1; $page<=$total_page; $page++){
        ...
        $p='#<tr>'.
            ...
            ' <td>(?<constnm>[^<]*)</td>'. //공고명
            ' <td>(?<org>[^<]*)</td>'. //발주처
            ' <td>(?<registdt>.*) <br />(?<closedt>.*)<br />(?<constdt>[^<]*)</td>'. //날짜
            ' <td>(?<contract>[^<]*)<br />[^<]*</td>'.
            ' <td>(?<basic>[^<]*)'.
